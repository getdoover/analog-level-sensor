//! Analog level sensor — Rust port of the Python `analog-level-sensor`, built
//! on the `doover` client library (doover-rs).
//!
//! Each loop it reads an analog-input pin (mA) from the platform-interface
//! sidecar, converts it to level (m) / percentage / volume using the
//! deployment config, and publishes them as tags. A reading below
//! `sensor_minimum_ma` (4 mA) is treated as a disconnected/faulted sensor and
//! skipped — faithful to the Python app.
//!
//! Config comes from the `deployment_config` channel (keyed by `APP_KEY`), so
//! no config file is needed in production.
//!
//! Env: `DDA_URI` (default 127.0.0.1:50051), `PLT_URI` (127.0.0.1:50053),
//! `APP_KEY`, and `SIMULATE_AI` (test only — synthesise a noisy in-range mA
//! value instead of reading the platform interface, to exercise the loop /
//! live-mode ceiling without the ~200 ms platform serial round-trip).
...
struct AnalogLevelSensor {
    ...
    /// Test mode (SIMULATE_AI): synthesise a noisy in-range mA value instead of
    /// the slow platform read, to expose the loop / live-mode ceiling.
    simulate: bool,
    rng: u64,
    t: u64,
}
...
impl Application for AnalogLevelSensor {
    ...
    async fn setup(&mut self, ctx: &AppContext) -> Result<()> {
        ...
        plt.test_comms("hello from analog-level-sensor (rust)").await?;
        self.plt = Some(plt);
        tracing::info!(
    ...
    async fn main_loop(&mut self, ctx: &AppContext) -> Result<()> {
        ...
        let c = &self.cfg;
        tracing::info!("Level sensor reading: {ma:.3} mA (AI pin {})", c.ai_pin);
