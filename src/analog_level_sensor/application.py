...
from .app_state import AnalogLevelSensorState

log = logging.getLogger()

class AnalogLevelSensorApplication(Application):
    config: AnalogLevelSensorConfig  # not necessary, but helps your IDE provide autocomplete!

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.started: float = time.time()
        self.ui: AnalogLevelSensorUI = None
        self.state: AnalogLevelSensorState = None

    async def setup(self):
        self.ui = AnalogLevelSensorUI()
        self.state = AnalogLevelSensorState()
